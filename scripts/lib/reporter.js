...
function generateCascadeHealthIssue(providerHealth, totalFeatures) {
    ...
    body += `| Provider | Queries | Errors | Error Rate | Last Error |\n`;
    body += `|---|---|---|---|---|\n`;
    ...
    for (const [name, stats] of Object.entries(providerHealth)) {
        ...
        const status = queriesUsed > 0 && (stats.errors / queriesUsed) > 0.8 ? '**' : '';
        const lastErr = stats.lastError ? stats.lastError.substring(0, 80) : '—';
        body += `| ${status}${name}${status} | ${queriesUsed} | ${stats.errors} | ${errorRate}% | ${lastErr} |\n`;
    }

    body += `\nProviders in **bold** are degraded (>80% error rate).\n`;
